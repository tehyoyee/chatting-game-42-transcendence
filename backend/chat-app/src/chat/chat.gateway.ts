...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect{
  ...
  async onCreateDmChannel(
    ...
    if (!user) {
      ...
      // this.logger.debug('Unidentified User');
      // throw new HttpException('Unidentified User', HttpStatus.UNAUTHORIZED);
      client.emit('create-dm-fail', 'Unidentified User Error in onCreateDmChannel');
      return ;
    }
    ...
    if (exist) {
      ...
      client.emit('create-dm-fail', 'dmRoom already exists Error in onCreateDmChannel');
      return ;
    }
    ...
    if (!receiver) {
      ...
      client.emit('create-dm-fail', 'receiver not found Error in onCreateDmChannel');
      return ;
    }
    ...
    client.emit('create-dm-success', newChannel.channel_id);
    receiverSocket.emit('create-dm-success', newChannel.channel_id);
