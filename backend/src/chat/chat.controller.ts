...
export class ChatController {
  ...
  async getAllPublicChannel(): Promise<Channel[]> {
    ...
    channels = await this.chatService.getAllGroupChannelsByChannelType(
      ChannelType.PUBLIC,
    );
  ...
  async getJoinedGroupChannelsByUserId(
  ...
  async getJoinedDmChannelsByUserId(
    @Param('id', ParseIntPipe) id: number,
  ): Promise<Channel[]> {
    let channels: Channel[] = [];
  ...
  @Get('profile/:id')
  async getProfileByUserIdInChannel(
    @Param('id', ParseIntPipe) id: number,
  ): Promise<User> {
