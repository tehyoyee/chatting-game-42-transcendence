...
export class ChatService {
  ...
  async createGroupChannelAndBridge(
    user: User,
    groupChannelDto: GroupChannelDto,
  ): Promise<Channel> {
    const newChannel =
      await this.channelRepository.createGroupChannel(groupChannelDto);
    await this.createUCBridge(user, newChannel, UserType.OWNER);

    return newChannel;
  }

  async createDmChannelAndBridges(
    sender: User,
    senderId: number,
    receiverId: number,
  ): Promise<Channel> {
    const newChannel = await this.channelRepository.createDmChannel(
      senderId,
      receiverId,
    );
  ...
  async getAllGroupChannelsByChannelType(
  ...
  async getJoinedGroupChannelsByUserId(userId: number) {
    ...
    for (const c of channelIds) {
      const tmp = await this.channelRepository.getChannelById(c.channel_id);
      if (
        tmp.channel_type === ChannelType.PUBLIC ||
        tmp.channel_type === ChannelType.PROTECTED
      ) {
  ...
  async createGroupMessage(
    ...
    return await this.messageRepository.createGroupMessage(
      sender,
      channel,
      content,
    );
  ...
  async getAllMessagesExceptBlockByChannelId(
    userId: number,
    channelId: number,
  ): Promise<PreviousMessageDto[]> {
    const previousMessages: PreviousMessageDto[] = [];
    ...
    while (i < previousMessages.length) {
      if (
        await this.relationService.checkBlocked(
          userId,
          previousMessages[i].writerId,
        )
      ) {
  ...
  async deleteChannelIfEmpty(channelId: number) {
    ...
    const bannedUsersId: number[] = [];
    for (const c of channels) {
      bannedUsersId.push(c.user_id);
    }
    for (const bId of bannedUsersId) {
  ...
  async updateUserTypeOfUCBridge(
    targetUserId: number,
    channelId: number,
    newType: UserType,
  ) {
    await this.ucbRepository.updateUserTypeOfUCBridge(
      targetUserId,
      channelId,
      newType,
    );
  ...
  async isOwnerOfChannel(userId: number, channelId: number) {
    ...
    if (!found) throw new HttpException('Unexist Bridge', HttpStatus.NOT_FOUND);

    if (found.user_type === UserType.OWNER) return true;
    return null;
  }
  ...
  async isAdminOfChannel(userId: number, channelId: number) {
    ...
    if (found.user_type === UserType.ADMIN) return true;
    return null;
  }

  async checkUserInThisChannel(
    userId: number,
    channelId: number,
  ): Promise<UserChannelBridge> {
  ...
  async getAllUsersInChannelByChannelId(
    newUserId: number,
    channelId: number,
  ): Promise<BridgeDto[]> {
    const inners: BridgeDto[] = [];
    const isBanned = false;
    ...
    for (const b of bridges) {
      ...
      const is_friend = await this.relationService.checkFriended(
        newUser.user_id,
        oldUser.user_id,
      );
      const is_blocked = await this.relationService.checkBlocked(
        newUser.user_id,
        oldUser.user_id,
      );
      const oldUserStatus = await this.userService.getCurrentUserStatusByUserId(
        b.user_id,
      );
