...
export class AuthService {
  ...
  async signUp(code: string, res: Response) {
    const generateRandomString = async (len: number) => {
      ...
      const checkDuplicate =
        await this.userService.getProfileByNickName(randomString);
      if (checkDuplicate) {
    ...
    const accessToken = await firstValueFrom(
      this.httpService
        .post(
          `https://api.intra.42.fr/oauth/token?grant_type=${grant_type}&client_id=${client_id}&client_secret=${client_secret}&code=${code}&redirect_uri=${redirect_uri}`,
        )
        .pipe(),
    );
    ...
    const user = await firstValueFrom(
      this.httpService.get('https://api.intra.42.fr/v2/me', axiosConfig).pipe(),
    );
    if (!user) {
      throw new HttpException('intra error', HttpStatus.BAD_REQUEST);
    }
    ...
    if (found) {
      const duplicateTest = await this.userService.getProfileByUserId(
        user.data.id,
      );
      if (duplicateTest) {
        if (duplicateTest.status !== UserStatus.OFFLINE) {
          throw new HttpException(
            'User already logged in this site.',
            HttpStatus.UNAUTHORIZED,
          );
        }
      }
      const two_factor = await this.userService.getTwoFactorByUserId(
        payload.id,
      );
      if (two_factor) {
        // 2차인증 ON & 2차인증 안한상태 => 메일보내기
        const clientEmail = await this.userService.getEmailByUserId(payload.id);
        const verificationCode =
          this.mailService.secondAuthentication(clientEmail);
        if (!verificationCode) {
          throw new HttpException('mailing error', HttpStatus.BAD_REQUEST);
        }
        const signedVerificationCode = this.jwtService.sign({
          verificationCode,
        });
        this.userService.updateTwoFactorCode(
          payload.id,
  ...
  async checkLoginState(req: Request, res: Response) {
    ...
    try {
      ...
      throw new HttpException(
        '[Login State] Unauthorized Token',
        HttpStatus.UNAUTHORIZED,
      );
    }
    const { payload } = this.jwtService.verify(token);
